...
#pragma warning restore CS0169 // The field 'HealingDrone.currentMovement' is never used
    ...
    //    Vector3 temporaryLookVector = new Vector3(0, 0, 0);

    //    if (moveVector.magnitude > Deadzone)
    //    {

    //        temporaryLookVector = moveVector;
    //    }
    ...
    void Update()
    {
        ...
        Vector3 lookVector = PlayerController.lookVector;

        movement = PlayerController.lookVector.normalized;

        if (PlayerController.lookVector.magnitude < PlayerController.deadZones[0])
        {
